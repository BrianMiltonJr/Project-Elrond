...
public class VitalityPrompt extends NumericPrompt{

	@Override
	public String getPromptText(ConversationContext con) {
		return "---------------------------------\n"
				+ "Well " + con.getSessionData("name") + " how many points would you like to add to Vitality? You have " + con.getSessionData("stp") + " Stat Points left.";
	}
	...
	protected Prompt acceptValidatedInput(ConversationContext con, Number num) {
		if(Integer.valueOf(num.toString()) > Integer.valueOf(con.getSessionData("stp").toString())){
			...
			con.setSessionData("vit", spendingStp);
			con.setSessionData("stp", newStp);
			return new StatLogicBufferPrompt(new SpiritPrompt());
		}
	}
