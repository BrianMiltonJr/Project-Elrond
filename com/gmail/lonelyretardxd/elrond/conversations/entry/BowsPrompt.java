...
public class BowsPrompt extends NumericPrompt{

	@Override
	public String getPromptText(ConversationContext con) {
		return "---------------------------------\n"
				+ "Well " + con.getSessionData("name") + " how many points would you like to add to Bows? You have " + con.getSessionData("sp") + " Skill Points left.";
	}
	...
	protected Prompt acceptValidatedInput(ConversationContext con, Number num) {
		if(Integer.valueOf(num.toString()) > Integer.valueOf(con.getSessionData("sp").toString())){
			...
			con.setSessionData("bows", spendingSp);
			con.setSessionData("sp", newSp);
			return new SkillLogicBufferPrompt(new LightArmorPrompt());
		}
	}
